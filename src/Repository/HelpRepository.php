...
class HelpRepository extends ServiceEntityRepository
{
    ...
    /**
     * Récupère la liste des demandes prise en charge par l'utilisateur
     * @return Help[]
     */
    public function getListDelivererHelps(User $user){
        ...
        $query= $this   
                ...
                ->andWhere("c.deliverer = :user AND c.isCancel = false AND c.dateHelp <= :date")
     ...
     * @return Help[]
     */
    public function findOnlineHelpsForUser(User $user){
        $todayWithHours = new DateTime();
        $today = $todayWithHours->format("Y-m-d");

        $query= $this   
                ->createQueryBuilder('p')
                ->andWhere("p.dateHelp >= :today AND p.user = :user")
                ->orderBy('p.dateHelp','DESC')
    ...
    {   
        ...
        $query= $this   
            ->createQueryBuilder('p')
            ->leftJoin("p.city", "c")
            ->andWhere("p.deliverer is null AND c is not null AND p.dateHelp >= :today ")
     ...
     * @return Help[]
     */
    public function findHelpsWithDeliverer(User $user): array
    {   
        $date = new DateTime();
        ...
        $query= $this   
            ->createQueryBuilder('p')
            ->andWhere("p.deliverer is not null AND p.user = :user AND p.dateHelp >= :date AND p.isDelivered = false")
            ->setParameter("user", $user)
     ...
     * @return Help[]
     */
    public function findMyHelpsWithoutDeliverer(User $user): array
    {
    ...
    public function findWaste(): array
    {
        ...
        $query = $this  ->createQueryBuilder('a')
                        ->andWhere("a.dateHelp < :date AND a.deliverer is null")
    ...
    public function findHistorical(User $user): array
    {
        ...
        $query = $this  ->createQueryBuilder('a')
                        ->andWhere("(a.isDelivered = true OR (a.dateHelp < :date AND a.deliverer is not null)) AND a.user = :user")
                        ->setParameter(':date', $dateWithoutHours)
                        ->setParameter(':user', $user);
